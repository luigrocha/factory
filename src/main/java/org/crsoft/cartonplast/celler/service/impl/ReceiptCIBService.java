...
public class ReceiptCIBService implements IReceiptService {

    private final ReceiptGeneratorUtil receiptGeneratorUtil;

    @Override
    public String getReceiptType() {
        return ReceiptType.CIB.getType();
    }

    @Override
    public byte[] generateReceipt(GenerateReceiptReq generateReceiptReq) {
        try {
            JasperReport jasperReport = receiptGeneratorUtil.getReportFromResources(ReceiptConstant.CIB_JRXML_NAME);
            ...
            if (jasperPrint == null) {
                throw new ReceiptGeneratorException("Error al generar el reporte CIB");
            }

            return JasperExportManager.exportReportToPdf(jasperPrint);
        } catch (JRException e) {
            log.error("Error generating receipt", e);
            throw new ReceiptGeneratorException("No se pudo generar el reporte", e);
        }
    }
}
