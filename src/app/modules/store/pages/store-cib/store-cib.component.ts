...
export class StoreCibComponent implements OnInit {
  ...
  getAllReasons() {
    this.cellerService.getAllOptionsByDocumentCode(DocumentEnum.CIB).subscribe((reasons => {
  ...
  generateReceipt(body: GenerateReceipt) {
    this.cellerService.generateReceiptPreview(DocumentEnum.CIB, body).subscribe(
      (data => {
